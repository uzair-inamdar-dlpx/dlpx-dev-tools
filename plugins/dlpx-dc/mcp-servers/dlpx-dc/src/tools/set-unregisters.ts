...
export function createSetUnregistersTool(ctx: ToolContext): ToolDef {
  const inputSchema = z.object({
    ...
    days: z
      ...
      .positive()
      .describe(
        "Number of days from now until the VMs are unregistered/slept.",
      ),
    vm_names: z
  ...
  return {
    ...
    description:
      "Takes a list of VMs and extends the un-registration time on each of them. Wraps `dc set-unregisters [--ignore-missing] <days> <vm_name>...`. dcol1 and dcol2 only (non-AWS).",
    inputSchema,
    handler: async (raw) => {
      const { target, days, vm_names, ignore_missing } =
        inputSchema.parse(raw);
      const argv = ["dc", "set-unregisters"];
      if (ignore_missing) argv.push("--ignore-missing");
