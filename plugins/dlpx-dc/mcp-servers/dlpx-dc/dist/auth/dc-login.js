...
export function isLoginRequiredError(r) {
    if (r.code === 0)
        return false;
    const haystack = `${r.stderr}\n${r.stdout}`;
    return LOGIN_REQUIRED_PATTERNS.some((p) => p.test(haystack));
}
export async function runWithDcLogin(run, creds, argv) {
    const first = await run(argv);
    if (!isLoginRequiredError(first))
        return first;
    ...
    if (login.code !== 0) {
        throw new Error(`dc login failed (exit ${login.code}): ${login.stderr.trim() || login.stdout.trim()}`);
    }
    return run(argv);
}
//# sourceMappingURL=dc-login.js.map