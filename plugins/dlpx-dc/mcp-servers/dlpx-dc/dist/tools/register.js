...
export function createRegisterTool(ctx) {
    const inputSchema = z.object({
        ...
        esx_host: z
            .string()
            .optional()
            .describe("Pin the VM to a specific ESX host (`--esx-host`). Defaults to a random host."),
        no_claiming: z
            .boolean()
            .optional()
        ...
        subnet: z
            .string()
            .optional()
            .describe("Assign the VM's interfaces to a named subnet (`--subnet`). Defaults to the default subnet."),
    });
    return {
        name: "dlpx_register",
        description: "Register a previously-unregistered VM to an ESX host and power it on (wraps `dc register [opts] <vm_name>`). dcol1 and dcol2 only.",
        inputSchema,
        handler: async (raw) => {
